...
-----12-----
SELECT DATEDIFF(DAY, p.data,  s.dataemissao) as 'Diferença em dias',
DATEDIFF(MONTH, p.data,  s.dataemissao) as 'Diferença em meses' 
FROM solicitacao s
...
----- Atualização de campos (atributos) da tabela -----
...
-- DROP - exclui estrutura, ou seja, todo conteúdo da tabela
...
4)DELETE nota
/*
Pois cotação possue chaves estrangeiras (FK), então deve-se excluir ela primeiro, 
depois tem que ser excluída a tabela pedido pois ela também possui chave estrangeira (FK), 
depois exclui-se a tabela solicitação ou a nota que não possuem FK.
Exclui-se primeiro as tabelas de movimentação (tabelas que possuem FK),
depois exclui-se as outras tabelas que não possuem FK.
*/
...
/* É apresentado erro e não exclui, devido ao fato de que nessa situação para podermos
...
para então deletarmos os registros da tabela nota. */
...
-- Conhecer os diagramas conceitual, lógico e físico;
-- Conhecer a sintaxe, CREATE TABLE (PK e FK), INSERT;
-- SELECT, UPDATE, DELETE;
-- Relacionamento da chave primária (PK) com a chave estrangeira (FK);
-- Sintaxe inner join;
-- Incluir ou excluir campo ALTER TABLE (ALTER COLUMN, ADD, DROP COLUMN);
-- Funções utilizadas:
-- TOP, delimita o número de registros para mostrar na tabela;
...
-- Aumentar 10% no valor unitário da tabela nota com UPDATE
UPDATE nota
