...
--------------------------------------------------------- Exercícios dia 09/06/2022 - Resoluções corrigidas no dia 24/06/2022 ---------------------------------------------------------

/* Resolução dos Exercícios de Cursores */

/* 1) Criar uma tabela aluno com os campos codigo, nome, nota1, nota2, qtdefaltas e qtdeaulas
inserir 5 registros na tabela e mostrar com uso de cursor todos os nomes dos alunos, e se a nota1
mais nota2 divido por dois é maior ou igual a 6 mostrar aprovado, se menor de quatro reprovado,
senão recuperação. Também verificar se o aluno não está reprovado por falta com a seguinte
regra a quantidade de falta for maior que 20% da quantidade de aulas o aluno está reprovado por
falta. */
...
--- LAÇO DE REPETIÇÃO ENQUANTO VERDADEIRO
WHILE @@FETCH_STATUS = 0 
	BEGIN
		IF ((@NOTA1+@NOTA2)/2) >= 6 
			BEGIN
...
percentual de IPI, inserir 5 registros na tabela e mostrar com uso de cursor todos os produtos da
UF SP, PR, SC o nome o valor e os percentuais de ICMS e IPI, bem como os respectivos valores
(valor multiplicado pelo percentual de ICMS e IPI). No final apresentar o valor total dos
produtos e os valores totais de ICMS e IPI. */
...
FETCH NEXT FROM PRODUTO_CURSOR INTO @CODIGO, @NOME, @UF, @CIDADE,@VALOR,@ICMS,@IPI

---- LAÇO DE REPETIÇÃO
WHILE @@FETCH_STATUS = 0 
BEGIN
	PRINT (' CODIGO = ' + CONVERT(VARCHAR(20),@CODIGO))
	PRINT (' NOME = ' + @NOME)
	PRINT (' UF = ' + @UF)
	PRINT (' CIDADE = ' + @CIDADE)
	PRINT (' VALOR DO PRODUTO = ' + CONVERT(VARCHAR(20),@VALOR))
	PRINT (' PERCENTUAL ICMS = ' + CONVERT(VARCHAR(10),@ICMS))
	PRINT (' VALOR DO ICMS = ' + CONVERT(VARCHAR(20), ((@VALOR * @ICMS)/100)))
	PRINT (' PERCENTUAL IPI = ' + CONVERT(VARCHAR(10),@IPI))
	PRINT (' VALOR DO IPI = ' + CONVERT(VARCHAR(20), ((@VALOR * @IPI)/100)))
	SET @VTOTAL = @VTOTAL + @VALOR
	SET @VTOTICM = @VTOTICM + ((@VALOR * @ICMS)/100)
	SET @VTOTIPI = @VTOTIPI + ((@VALOR * @IPI)/100)
	FETCH NEXT FROM PRODUTO_CURSOR INTO @CODIGO, @NOME, @UF, @CIDADE, @VALOR,
	@ICMS, @IPI
END
PRINT ( ' VALOR TOTAL DOS PRODUTOS = ' + CONVERT(VARCHAR(20),@VTOTAL))
PRINT ( ' VALOR TOTAL DE ICMS = ' + CONVERT(VARCHAR(20),@VTOTICM))
PRINT ( ' VALOR TOTAL DE IPI = ' + CONVERT(VARCHAR(20),@VTOTIPI))
...
/* 3) Criar uma tabela aluno com os campos codigo, nome, sexo, UF, cidade, nota1, nota2, curso,
mensalidade inserir 5 registros na tabela e mostrar com uso de cursor quantos alunos são de cada
curso e quantos são mulheres e homens, bem como o valor total das mensalidades dos alunos por
curso. Para os alunos que tem média acima 8 conceder um desconto de 10% na mensalidade. */
...
--- DECLARAR O CURSOR
DECLARE ALUNO_CURSOR CURSOR FOR
SELECT CODIGO,NOME,NOTA1,NOTA2,MENSALIDADE FROM ALUNO
OPEN ALUNO_CURSOR
DECLARE @CODIGO INT
DECLARE @NOME VARCHAR(40)
DECLARE @NOTA1 NUMERIC(5,2)
DECLARE @NOTA2 NUMERIC(5,2)
DECLARE @MENSALIDADE NUMERIC(12,2)
FETCH NEXT FROM ALUNO_CURSOR INTO @CODIGO, @NOME, @NOTA1, @NOTA2,
@MENSALIDADE
...
BEGIN
	IF ((@NOTA1+@NOTA2) / 2) > 8 
		BEGIN
		UPDATE ALUNO SET MENSALIDADE = @MENSALIDADE * 0.9 WHERE CODIGO =@CODIGO
		END
	FETCH NEXT FROM ALUNO_CURSOR INTO @CODIGO, @NOME, @NOTA1, @NOTA2,
	@MENSALIDADE
END
...
--- OUTRO CURSOR PARA TRAZER O RESUMO DAS TURMAS
DECLARE TURMA_CURSOR CURSOR FOR
SELECT CURSO,SEXO,COUNT(SEXO),SUM(MENSALIDADE) FROM ALUNO
GROUP BY CURSO,SEXO

OPEN TURMA_CURSOR
DECLARE @CURSO VARCHAR(3)
DECLARE @SEXO VARCHAR(1)
DECLARE @QTDEHM INT
DECLARE @TOTMEN NUMERIC(12,2)

FETCH NEXT FROM TURMA_CURSOR INTO @CURSO,@SEXO,@QTDEHM,@TOTMEN

--- LAÇO DE REPETIÇÃO
WHILE @@FETCH_STATUS = 0 
BEGIN
	PRINT (' CURSO = ' +@CURSO + ' SEXO = ' + @SEXO + ' QTDE = ' +
	CONVERT(VARCHAR(10),@QTDEHM) + ' TOTAL MENSALIDADE = ' +
	CONVERT(VARCHAR(15),@TOTMEN))
	FETCH NEXT FROM TURMA_CURSOR INTO @CURSO,@SEXO,@QTDEHM,@TOTMEN
END

---- FECHA CURSOR
CLOSE TURMA_CURSOR
DEALLOCATE TURMA_CURSORV

-----------------------------------------------------------------------------------------------------

/* 4) A partir das tabelas departamento e funcionário faça o relacionamento e desenvolva usando
cursor em programa em PL/SQL que mostre o nome do funcionário, departamento, salário total,
valor de FGTS e Valor do INSS, no final apresentar o total geral do salário, do FGTS e INSS,
apenas dos departamentos 3 e 2, cuja a quantidade de horas seja maior que 200 e menor que 360 e o
mês referente a data seja 2 e 4 apenas. */
...
INSERT INTO DEPARTAMENTO VALUES ('TI',240)
INSERT INTO FUNCIONARIO VALUES ('PEDRO',3,10.5,'01-04-2020',5,17)
INSERT INTO FUNCIONARIO VALUES ('JOAO',2,12.3,'20200315',8,8)
INSERT INTO FUNCIONARIO VALUES ('MANUEL',2,16.8,'20200222',7,15)
INSERT INTO FUNCIONARIO VALUES ('PATRICIA',1,20.7,'20200327',4,7)
INSERT INTO FUNCIONARIO VALUES ('SALETE',3,18.8,'20200412',10,12)

DECLARE FUNC_CURSOR CURSOR FOR
SELECT F.FUNC_NOME,D.DEPTO_NOME,(F.FUNC_SALARIO_HORA*D.DEPTO_QTDE_HORAS),
((F.FUNC_SALARIO_HORA*D.DEPTO_QTDE_HORAS)*F.FUNC_FGTS)/100,((F.FUNC_SALARIO_HORA*D.DEPTO_QTDE_HOR
AS)*F.FUNC_INSS)/100 
FROM FUNCIONARIO F
INNER JOIN DEPARTAMENTO D
ON F.FUNC_DEPTO = D.DEPTO_COD
AND D.DEPTO_QTDE_HORAS > 200 
AND D.DEPTO_QTDE_HORAS < 360 
WHERE F.FUNC_DEPTO IN (2,3)
AND MONTH(F.FUNC_DATA) IN (2,4)

DECLARE @NOME VARCHAR(40)
DECLARE @NDEPTO VARCHAR(40)
DECLARE @SALTOT NUMERIC(12,2)
DECLARE @VLRFGTS NUMERIC(12,2)
DECLARE @VLRINSS NUMERIC(12,2)
DECLARE @TSALTOT NUMERIC(12,2)
DECLARE @TVLRFGTS NUMERIC(12,2)
DECLARE @TVLRINSS NUMERIC(12,2)
SET @TSALTOT = 0 
SET @TVLRFGTS = 0 
SET @TVLRINSS = 0

OPEN FUNC_CURSOR
FETCH NEXT FROM FUNC_CURSOR INTO @NOME,@NDEPTO,@SALTOT,@VLRFGTS,@VLRINSS

WHILE @@FETCH_STATUS = 0 
BEGIN
	PRINT (' FUNCIONARIO :'+ @NOME)
	PRINT (' DEPARTAMENTO:'+ @NDEPTO)
	PRINT (' SALARIO TOTAL:'+ CONVERT(VARCHAR(15),@SALTOT))
	PRINT (' VALOR DE FGTS:'+ CONVERT(VARCHAR(15),@VLRFGTS))
	PRINT (' VALOR DE INSS:'+ CONVERT(VARCHAR(15),@VLRINSS))
	SET @TSALTOT = @TSALTOT + @SALTOT
	SET @TVLRFGTS = @TVLRFGTS + @VLRFGTS
	SET @TVLRINSS = @TVLRINSS + @VLRINSS
	FETCH NEXT FROM FUNC_CURSOR INTO @NOME,@NDEPTO,@SALTOT,@VLRFGTS,@VLRINSS
END
PRINT (' SALARIO TOTAL GERAL :' + CONVERT(VARCHAR(15),@TSALTOT))
PRINT (' VALOR TOTAL FGTS :' + CONVERT(VARCHAR(15), @TVLRFGTS))
PRINT (' VALOR TOTAL INSS :' + CONVERT(VARCHAR(15),@TVLRINSS))
CLOSE FUNC_CURSOR
